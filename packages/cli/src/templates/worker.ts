...
/**
 * Generates a full standalone Cloudflare Worker that proxies Supabase traffic.
 * Ports all capabilities from the JioBase proxy (HTTP, WebSocket, CORS, service filtering)
 * with hardcoded config for single-project self-hosting.
 */
const VALID_SERVICES = new Set(['rest', 'auth', 'storage', 'realtime', 'functions', 'graphql']);

export function generateWorkerCode(config: WorkerConfig): string {
  // Sanitize: only allow known service names (defense-in-depth)
  const safeServices = config.enabledServices.filter((s) => VALID_SERVICES.has(s));
  const services = safeServices.map((s) => `'${s}'`).join(', ');
...
// --- Config ---
const SUPABASE_URL = ${JSON.stringify(config.supabaseUrl)};
const ALLOWED_ORIGINS = ${JSON.stringify(config.allowedOrigins)};
const ENABLED_SERVICES: SupabaseService[] = [${services}];
